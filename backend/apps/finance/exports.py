"""
finance/exports.py — CSV export of the admin invoice list (Colegiaturas).

Mirrors the cafetería export pattern (apps/cafeteria/exports.py): es-MX headers
and a UTF-8 BOM so Excel opens accented names correctly.
"""
import csv

from django.http import HttpResponse
from django.utils import timezone


def _fmt_dt(dt) -> str:
    if not dt:
        return ''
    return timezone.localtime(dt).strftime('%Y-%m-%d %H:%M')


def _filename(prefix: str) -> str:
    stamp = timezone.localtime(timezone.now()).strftime('%Y%m%d')
    return f'{prefix}_{stamp}.csv'


def invoices_csv(invoices) -> HttpResponse:
    """CSV of the (already filtered) admin invoice queryset."""
    header = ['Alumno', 'Matrícula', 'Grado', 'Periodo', 'Vence',
              'Monto', 'Pagado', 'Saldo', 'Estado']

    response = HttpResponse(content_type='text/csv; charset=utf-8')
    response.write('﻿')  # BOM so Excel opens UTF-8 accents correctly
    writer = csv.writer(response)
    writer.writerow(['Colegiaturas'])
    writer.writerow(['Generado', _fmt_dt(timezone.now())])
    writer.writerow([])
    writer.writerow(header)
    for inv in invoices:
        student = inv.student
        writer.writerow([
            student.user.full_name,
            student.student_id,
            f'{student.grade} {student.group}'.strip(),
            inv.period,
            inv.due_date.isoformat() if inv.due_date else '',
            f'{inv.amount:.2f}',
            f'{inv.amount_paid:.2f}',
            f'{inv.balance_due:.2f}',
            inv.get_status_display(),
        ])
    response['Content-Disposition'] = f'attachment; filename="{_filename("colegiaturas")}"'
    return response
