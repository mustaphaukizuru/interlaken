...
# Behind Caddy, TLS is terminated upstream; trust the forwarded
...
# so under gunicorn's multiple worker processes each worker keeps its OWN
# counter bucket and the effective ceiling is multiplied by the worker count.
# Back the counters with a host-shared file cache (no Redis on the VPS;
