...
**16. Publishing an announcement fans out nothing (no in-app/email/push to the audience)** — ✅ **FIXED 2026-07-18 (in-app)**: publishing an active comunicado now bulk-creates an in-app Notification for every user in the audience (fail-soft). _Remaining:_ the email + web-push blast to the full audience is deferred to a batched cron (a synchronous school-wide send would time out on this queue-less host); and a draft published later (is_active False→True) does not yet fan out.  
