...
## Service worker gate (`npm run check:sw`)

The PWA worker (`dist/sw.js`, generated by vite-plugin-pwa) is checked in CI
right after the budgets. It must **not** contain a `NavigationRoute` /
`createHandlerBoundToURL` (generateSW's default `navigateFallback`), must keep
excluding `/api`, `/admin`, `/auth`, `/static` and `/media` from its page route,
and must import `push-sw.js`.

Why: on 2026-08-21 a build shipped with the default `navigateFallback`, which
answers *every* navigation with the SPA shell in any browser that has the
worker installed. The backend redirect to `/auth/google/` therefore never left
the browser and users saw the React 404 page when signing in with Google; the
Django admin at `/admin/` broke the same way. `vite.config.ts` sets
`navigateFallback: null`; `frontend/scripts/check-sw.mjs` keeps it that way.

If a user reports "404 Página no encontrada" at `/auth/google/` or `/admin/`
again, the first question is whether the deployed `https://interlaken.edu.mx/sw.js`
contains `NavigationRoute` (`curl -s https://interlaken.edu.mx/sw.js | grep -c
NavigationRoute` should print `0`).
