...
| P3-3 | Phase 2: Page, PageVersion, blocks JSON, block schema registry (`frontend/src/cms/blocks/*.schema.ts` generating backend validator + editor form), public cached API with ETag, `<CmsPage>` renderer, 6 core blocks (hero, stats, levels_cards, rich_text, image, cta_band), Home ported. | Partial 2026-08-22 (backend: Page/PageVersion, BLOCK_SCHEMAS validation, publish/unpublish/versions/rollback, signed preview, alt guardrail, cached public API + ETag; frontend: registry, sanitizer, BlockRenderer with 16 block types, /p/:slug and catch-all CMS routes. Home port waits for the seed P3-2 and the editor P3-4) |
