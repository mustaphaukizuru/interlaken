...
| P1-A8 | Roster table: column chooser, sort, nivel/grado/grupo filters, bulk actions, density toggle, sticky header, mobile cards. | DONE 2026-08-23 (server `?nivel/grado/grupo/ordering` with whitelist; sortable headers with aria-sort, URL-synced `orden`; column chooser + compact density persisted per browser; row checkboxes + bulk status/grupo/grado via `POST admin/students/bulk/` audited per student; sticky header and mobile cards already in place) |
...
| P1-B7 | Forms on mobile: single column, 44px inputs, inputmode/autocomplete, sticky submit, step indicator. | DONE 2026-08-23 (44px/16px inputs global, inputMode/autoComplete on tel/email, step indicator + sticky footer in the recarga wizard and pre-registro, CMS forms render single column with the same controls) |
...
| ID | Task | Status |
|----|------|--------|
| P1-C1 | Dedicated mailboxes and settings: `noreply@`, `admisiones@`, `info@`, `cafeteria@`, `pagos@`, `facturacion@`, `soporte@`, `privacidad@`, `direccion@`, `sistemas@`; SPF/DKIM/DMARC; Reply-To per feature. | DONE in code 2026-08-23 (settings CONTACT/ADMISSIONS/BILLING/SUPPORT/PRIVACY/OPS_EMAIL, Reply-To on every outbound mail, [STAGING] prefix). School/ops task pending: create the mailboxes in Hostinger and the DNS SPF/DKIM records, see docs/OPS-RUNBOOK.md |
...
| P1-C4 | Preferences UI: per-channel, per-category, quiet hours, digest vs instant, per-child. | DONE 2026-08-22 (per-channel + per-category cafetería/pagos/comunicados; warnings never muted; quiet hours and per-child deferred to P4) |
...
| P1-D4 | Payment detail drawer with reference and gateway; comprobante PDF download and email. | DONE 2026-08-22 (reference/gateway inline on each row; /payments/<id>/receipt/ PDF; email-the-receipt deferred) |
...
| P1-G3 | Agendar visita form enhancement. | DONE 2026-08-23 (mobile inputs from B7; open-class events block on Admisiones; admin weekly agenda with reschedule/outcome from P4-2; .ics download) |
...
| P2-7 | Nivel pages: images, icons, graphs, colors (Pre #8AC6A4, Prim #F4436C, Sec #F1C82F), full info, SEP incorporation. | DONE in code 2026-08-23 (official level colors, SEP card per level editable in Ajustes, responsive images). Content task pending: the school supplies per-level photos and copy, editable in the CMS without a deploy |
...
| P3-2 | Content seeding script: extract current copy and 26 images into version 1 of each page. | DONE 2026-08-22 (`manage.py seed_cms [--publish] [--force]`: imports frontend/public/assets into the media library by SHA-256 with alt text, creates draft pages inicio/nosotros/galeria/contacto from the current copy; idempotent, tested) |
| P3-3 | Phase 2: Page, PageVersion, blocks JSON, block schema registry (`frontend/src/cms/blocks/*.schema.ts` generating backend validator + editor form), public cached API with ETag, `<CmsPage>` renderer, 6 core blocks (hero, stats, levels_cards, rich_text, image, cta_band), Home ported. | DONE 2026-08-23 (backend Page/PageVersion, BLOCK_SCHEMAS, publish/unpublish/versions/rollback, signed preview, cached public API + ETag; frontend registry, sanitizer, BlockRenderer with 18 block types, /p/:slug and catch-all. Home port: `seed_cms` creates the draft and `CmsOverride` swaps it in the moment Dirección publishes it) |
| P3-4 | Phase 3: visual editor at `/admin/contenido`: split view, preview token iframe, edit mode (hover outlines, click select, inline text, drag reorder), autosave, publish, schedule, unpublish, version history with diff, rollback, device-width toggle. | DONE 2026-08-22 (core: `/admin/contenido` page list + create/delete, `/admin/contenido/:id` split-view editor: block list with add/reorder/duplicate/remove, registry-generated property forms with media picker, live preview with click-to-select and mobile/tablet/desktop widths, autosave 1.5 s, publish/unpublish, versions + rollback, SEO/slug panel, shareable preview link. 2026-08-23: drag-and-drop reorder in the block list (native HTML5, no dependency), version diff ('Ver cambios' per version: added / removed / modified / moved blocks vs the draft), scheduled publish shipped under P3-11. Inline text editing stays out: the property panel is the single editing surface) |
| P3-5 | Phase 4: remaining blocks (gallery, timeline, feature_grid, testimonials, faq, video, map_contact, pricing_table, form, open_school_events, calendar, sep_incorporation) and pages ported in traffic order: Admisiones, Costos, Nivel x3, Modelo, Nosotros, Galería, Contacto, Plataformas, Facturación, Documentación; delete hard-coded TSX. | DONE 2026-08-22 (all 18 block types render; seed_cms now creates 9 draft pages: inicio, nosotros, admisiones, costos, modelo-educativo, plataformas, documentacion, galeria, contacto; `CmsOverride` wraps the public routes so a *published* CMS page replaces the TSX with no deploy. Deliberately kept: the TSX pages as fallback until the school validates each port, NivelPage x3 and Facturación (form-driven) stay hard-coded) |
| P3-6 | Phase 5: forms builder (field types, conditional fields, consent, notify emails per mailbox), submissions inbox; port Contacto, Pre-registro, Facturación. Registration wizard and Book-visit stay code. | DONE 2026-08-22 (FormDefinition/FormSubmission, 9 field types, show_if conditions, consent checkbox, honeypot, per-form notify mailbox with Reply-To, `/admin/formularios` builder + submissions inbox with CSV and 'atendido', sidebar badge; `form` block renders CMS forms by slug. Kept in code by design: Pre-registro (eligibility + drafts + booking), Facturación (CFDI validation) and Contacto (existing inbox); they remain reachable from the form block) |
| P3-7 | Phase 6: menu editor (header, footer, mobile tab bar), per-page SEO + sitemap, redirects, EN locale (per D8). | DONE 2026-08-22 (SiteSettings.menu JSON edited at `/admin/navegacion`, one menu drives header dropdowns, mobile drawer and footer; `Redirect` model with 301/302, hit counter, resolved by the SPA catch-all; dynamic `/sitemap.xml` from Django = static routes + published CMS pages minus noindex/redirected, replaces the static file; per-page SEO already in the editor (P3-4). EN locale deferred per D8) |
| P3-8 | Site-wide snippets (address, hours, SEP) edited once. | DONE 2026-08-22 (address/hours/phone/email already lived in Ajustes; added `sep_incorporations` to SiteSettings with an Ajustes card, `useSep()` feeds Admisiones, each Nivel and the CMS block; rich text supports {{direccion}} {{horario}} {{telefono}} {{correo}} {{whatsapp}} tokens) |
| P3-9 | Public avisos banner driven by Comunicados "publicar en sitio". | DONE 2026-08-22 (Announcement.show_on_site/site_until/site_link, public `/portal/avisos/` cached 5 min and busted on save, amber banner above the public header with per-aviso dismissal, checkbox + dates in the Comunicados modal) |
| P3-10 | Shareable expiring preview links for Dirección approval; optional approval step (Comunicación drafts, Dirección publishes). | DONE 2026-08-22 (preview tokens last 7 days; `staff` role edits pages/media under `/staff/contenido`, requests approval (admins get in-app + email with preview link), Dirección publishes or 'Pedir cambios' with a note that reaches the editor; staff cannot publish/delete/rollback) |
| P3-11 | Scheduled unpublish for time-bound content. | DONE 2026-08-22 (Page.publish_at/unpublish_at in the SEO panel; `cms_schedule` command every 5 min in crontab.example; public view and sitemap also enforce unpublish_at lazily so expiry never waits for cron) |
| P3-12 | Pre-publish checks: broken links, missing alt. | DONE 2026-08-22 (`apps/content/checks.py`: required props, missing/alt-less images, broken internal links (routes + published pages + redirects), empty text/gallery, duplicate hero/cta/stats, SEO title/description length; `GET …/checks/` + 'Revisar' panel in the editor with click-to-block; publish refuses on errors and returns the issue list) |
| P3-13 | Guardrails: AuditLog on publish, cache busting, signed preview only, block schema migrations, form honeypot/rate limit/consent. | DONE 2026-08-22 (verified by tests: AuditLog `cms.publish`/`cms.unpublish`, page cache + sitemap busting on publish, signed 7-day preview tokens only, block registry mirrored backend/frontend with `validate_blocks`; added rate limits: form submit 5/min, redirect hit 30/min; honeypot + consent already enforced) |
| P3-14 | Flip `content.*` read-only in Django admin after phases 1 to 3. | DONE 2026-08-22 (Page, PageVersion, MediaAsset, FormDefinition, FormSubmission, Redirect registered read-only via `ReadOnlyAdmin`; pricing tables and SiteSettings stay editable there as the technical fallback) |
...
| P4-2 | Visitas: weekly calendar with drag reschedule, 24h reminders, no-show, post-visit outcome linked to pre-registro. | DONE 2026-08-23 (`/admin/visitas/semana` 7-day agenda from `GET bookings/admin/week/`; reschedule to another slot with capacity check, calendar re-sync and family email (kept as a picker instead of drag to stay usable on tablets); no-show one click; post-visit outcome (interesada / se inscribirá / no por ahora / no le interesó + note) marks attended and moves the family's pre-registro to contactado/inscrito/no procedió; 24h reminders already existed) |
| P4-3 | Cafetería: low-balance push with family threshold, weekly reminder or auto-recarga if gateway tokens allow, daily spend limit per child, monthly statement PDF, bulk top-up by group, reconciliation with Loyverse report. | DONE 2026-08-23 (already shipped: family threshold + low-balance alerts, daily/weekly spend limits, Loyverse reconcile report; added: monthly statement PDF per child (`GET cafeteria/statement/`, family-scoped) with a button on Cafetería, and admin 'Recarga masiva' by grado/grupo with preview, one audited ledger row + notification per family. Auto-recarga not built: gateways do not provide stored-card tokens in sandbox) |
| P4-4 | Comunicados: audience targeting, scheduling, read receipts, attachments/rich text, acknowledgement type, push on publish. | DONE 2026-08-23 (already: audience targeting, read receipts, push on publish; added: `publish_at` scheduling (hidden from families and fanned out by `cms_schedule` when due, once), `requires_ack` with 'Enterado' button and per-family `acknowledged_at`, ack count in the delivery report, attachments picked from the media library (validated, max 10) shown as download chips. Rich text intentionally kept as plain text for push/email parity) |
...
| P4-8 | Audit log UX: filters, diff view, export. | DONE 2026-09-25 (Data Ops Phase 7: `q`, action/actor/context/object type/object id/date filters in the URL, sort by date/actor email/action/object/context, row drawer with a before/after diff of `changes` and links to the admin page, "Historial de este objeto", CSV/XLSX/PDF export honouring the view and audited) |
| P4-9 | PWA: offline credencial and last comunicados, iOS install screen, app badges. | DONE 2026-08-23 (service worker `offline-essentials` cache keeps cafetería cards, comunicados and site settings for 7 days (NetworkFirst, 4 s timeout); `OfflineBanner` in the portal; `useAppBadge` mirrors unread notifications on the installed icon via the Badging API; iOS install instructions already in InstallHint) |
...
| P5-2 | Sentry activation, uptime check, backup restore drill documented. | DONE 2026-08-23 (docs/OPS-RUNBOOK.md: Sentry activation for Django + React with alert routing, UptimeRobot on /healthz + keyword check, incident table, operations calendar; `deploy/restore-db.sh` restores a dump into a scratch DB and prints counts (drill) or into the live DB with typed confirmation; monthly drill cron line. Activation itself needs the DSN and the monitor account, school/ops task) |
| P5-3 | Visual regression and Lighthouse budgets in CI. | DONE 2026-08-22 (Playwright visual baselines blocking in CI with win32 + linux PNGs committed, diff artifacts on failure; new `lighthouse` job runs Lighthouse CI on the built SPA for 5 public routes with score thresholds (perf 0.85, a11y 0.95, SEO 0.9, CLS 0.1, JS 400 KB) and a resource/timing budget file; documented in frontend/e2e/README.md) |
| P5-4 | Data retention policy and auto-purge job (pre-registros, submissions, documents, logs). | DONE 2026-08-22 (docs/RETENTION.md policy table; `apps/core/retention.py` + `purge_retention` command, dry-run by default, `--apply` anonymises rejected prospects/bookings and deletes handled submissions/messages, read notifications, login history, rejected-registration documents and expired tokens, audited; weekly cron entry; ledgers, student records and AuditLog stay human-reviewed via `report_retention`) |
| P5-5 | ARCO workflow status view with `privacidad@` intake. | DONE 2026-08-22 (`/admin/arco`: open/all/resolved/rejected tabs, 20-business-day deadline countdown with overdue alert, en revisión / resolver / rechazar with note; 'Registrar solicitud recibida' intake for requests arriving at privacidad@ / WhatsApp / in person (channel, requester, matched to the account when it exists) sending an acknowledgement with the statutory deadline; outcome emails with Reply-To privacidad@; sidebar badge; `PRIVACY_EMAIL` setting) |
| P5-6 | Minor data handling: medical fields role-restricted, masked in exports. | DONE 2026-08-22 (`apps/accounts/medical.py` policy: admin always, guardian only with MEDICAL_DATA consent for that student, staff/students never; student detail masks with `medical_masked` reason; `mask_medical()` for any export; roster CSV and AuditLog verified never to carry values; fields remain encrypted at rest) |
