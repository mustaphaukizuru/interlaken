...
- **R1 — Loyverse points as a wallet (validate FIRST):** using loyalty `total_points` as prepaid balance is a workaround. Confirm the Loyverse API actually allows **writing** `total_points` via `PATCH /customers/<id>` (`services.py:124`) — several Loyverse plans treat points as **read-only**, updated only through sales/redemptions. If writes aren't supported, the top-up mechanism must change (e.g., record credit locally and reconcile, or use a proper prepaid/redemption flow). **This gates Feature 4.**
  ...
  > 10. **History is complete and never double-charged.** A receipt dated on or before the wallet's `seeded_at` is stored with `applied=False` (shown to the family, no debit, no `balance_after`) by every path: poll, webhook, nightly window and replay. `backfill_receipt_history --days N --commit` fills older gaps for students whose balance equals Loyverse and parks receipts of cards without a student.
  > 11. **Recargas land at once.** The `customers.update` webhook re-reads each card from the API; a fresh value skips the settle window and is credited immediately unless it echoes a purchase (one, or the sum of those recorded in the last 15 min). Only a payload the API could not confirm is treated as stale.
  > 9. **Every card in the store is synced**, not only pupils. `LoyverseProfile` now has a row per Loyverse customer with `kind` (student/staff/test/other), live points and visits, and `missing_since` when Loyverse no longer has it; refreshed on each full pass and on `customers.update`. Staff-card wallet receipts are kept as `UnmatchedReceipt` rows and shown in the console (Clientes Loyverse tab, `admin/customers/`). The wallet ledger itself stays student-only: staff balances are read from Loyverse, never mirrored into `CafeteriaBalance`.
