...
export function StatCard({ title, value, suffix, icon: Icon, color, trend, subtitle }: StatCardProps) {
  const t = theme[color];
  return (
    <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 18, padding: '22px', boxShadow: t.shadow, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ marginTop: 4, display: 'flex', alignItems: 'baseline', gap: 4 }}>
        <span style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 36, letterSpacing: -1, color: 'var(--text-main)', lineHeight: 1.1 }}>{value}</span>
